        ...
        if (resource != null && resource.has("identifier")) {
            ...
            try {
                ...
                if (responseData != null && responseData.isOk()) {
                    ...
                    log.info(SoftwareCoMusic.getPluginName() + ": Unable to fetch latest commit info");
                }
            } catch (Exception e) {
                //
            }
    ...
    private void processCommits(JsonArray commits, String identifier, String tag, String branch) {
        try {
            ...
            if (resp != null && resp.getJsonObj() != null) {
                ...
                log.info(SoftwareCoMusic.getPluginName() + ": completed commits update - " + message);
            } else {
                log.info(SoftwareCoMusic.getPluginName() + ": Unable to process repo commits");
            }
        } catch (Exception e) {
            log.warning(SoftwareCoMusic.getPluginName() + ": Unable to process repo commits, error: " + e.getMessage());
        }
    }
            ...
            if (members.size() > 0) {
                ...
                try {
                    ...
                    if (responseData != null && responseData.has("message")) {
                        ...
                        log.info(SoftwareCoMusic.getPluginName() + ": " + responseData.get("data").getAsString());
                    } else {
                        log.info(SoftwareCoMusic.getPluginName() + ": Unable to process repo member metrics");
                    }
                } catch (Exception e) {
                    log.warning(SoftwareCoMusic.getPluginName() + ": Unable to process repo member metrics, error: " + e.getMessage());
                }
            }
        }
    }
